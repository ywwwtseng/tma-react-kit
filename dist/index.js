...
import { Layout, TabBar, useNavigate, useRoute } from "@ywwwtseng/react-kit";
...
function TMALayout({
  ...
  return /* @__PURE__ */ jsxs2(Layout.Root, { className: status !== 0 /* Loading */ ? "animate-fade-in" : "", style: styles?.root, children: [
    createPortal(
      /* @__PURE__ */ jsxs2(
        Layout.Header,
        {
...
import { useEffect as useEffect4, useState as useState3, useRef as useRef3 } from "react";
...
function TMA({
  ...
  const tabs = useMemo6(
    () => Object.entries(screens).filter(([_, screen]) => screen.icon).map(([name, screen]) => ({ name, title: screen.title, icon: screen.icon })),
    [screens]
  );
